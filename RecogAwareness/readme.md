...
![\hat{y}_k(x,\theta)=\sum_{m=1}^M\pi_m(x)\frac{\exp(a_j(x))}{\sum_{i=1}^K\exp(a_i(x))};s.t.\sum_{m=1}^M\pi_m=1](https://latex.codecogs.com/svg.latex?\hat{y}_k(x,\theta)=\sum_{m=1}^M\pi_m(x)\frac{\exp(a_j(x))}{\sum_{i=1}^K\exp(a_i(x))};s.t.\sum_{m=1}^M\pi_m(x)=1)
