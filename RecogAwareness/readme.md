...
![J=-\sum_{n=1}^N \log \hat{y}_{c_n}(x_n)](https://latex.codecogs.com/svg.latex?\hat{y}_k(x,\theta)=\sum_{m=1}^M\pi_{x,m}\frac{\exp(a_j(x))}{\sum_{i=1}^K\exp(a_i(x))};s.t.\sum_{m=1}^M\pi_{x,m}=1)
